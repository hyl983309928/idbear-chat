...
export function apiRequest(path, method = 'get', data = null) {
  if (process.env.NODE_ENV === 'test') {
    if (path.match(/^\/wxapi/)) {
      // path = window.location.protocol + '//' + window.location.host + path.replace('/wxapi', '')
      path = 'http://test.id-bear.com' + path.replace('/wxapi', '')
    }
  } else if ( process.env.NODE_ENV === 'production' ) {
    if (path.match(/^\/wxapi/)) {
      // path = window.location.protocol + '//' + window.location.host + path.replace('/wxapi', '')
      path = 'https://wechat.xxm3.id-bear.com' + path.replace('/wxapi', '')
    }
  }
  var url = path
  var headers = {
